...
void show_name()
{
    int i, j, x, y, star_id1, star_id2, star_x, star_x1, star_y1, star_x2, star_y2, write_x, write_y, pair_id = -1;
    char input_str[25], star_name[25], star_name1[25], star_name2[25], pair_full_name[55] = "\0";
    // input user's name
    do
    {   
        printf("Enter your full name: ");
    ...
    for (i = 0; i <= pair_number - 1; i++)
    {
        ...
        {
            pair_id = star_pairs[i].id;

            star_id1 = star_pairs[i].star1_id;
            strcpy(star_name1, star_pairs[i].star1_name);
            star_x1 = stars[star_pairs[i].star1_id].x;
            star_y1 = stars[star_pairs[i].star1_id].y;
      
            star_id2 = star_pairs[i].star2_id;
            strcpy(star_name2, star_pairs[i].star2_name);
            star_x2 = stars[star_pairs[i].star2_id].x;
            star_y2 = stars[star_pairs[i].star2_id].y; 
    ...
    strcat(pair_full_name, star_name1);
    strcat(pair_full_name, " & ");
    strcat(pair_full_name, star_name2);

    // finding the y coordinate to copy string
    write_y = (star_y1 <= star_y2) ? star_y2 + 1 : star_y1 + 1;
    write_x = (star_y1 <= star_y2) ? star_x2 - strlen(pair_full_name)/2 + 1 : star_x1 - strlen(pair_full_name)/2 + 1;
    if(write_y > max_y - 1)
    {
        write_y = (star_y1 <= star_y2) ? star_y1 - 1 : star_y2 - 1;
        write_x = (star_y1 <= star_y2) ? star_x1 - strlen(pair_full_name)/2 + 1 : star_x2 - strlen(pair_full_name)/2 + 1;
    }
    
    if (write_x + strlen(pair_full_name) - 1 > max_x -1)
    {
        write_x = max_x - 1 - strlen(pair_full_name) + 1; 
    }
    ...
    write_at(write_x, write_y, strcat(strcat(star_name1, " & "), star_name2)); 
...
int main()
{
    ...
    while (game_stop == 0)
    {
        ...
        // the pairs command
         else if (strcmp(game_command, "pairs") == 0)
        {
            list_pair();
        }
        // the draw command
